...
Dim cmdResultadoConf
	...
	If rsConfig("FLAG_TER") = true Then
		If Dia = 3 AND Hora > CDate(rsConfig("HR_TER_INICIO")) AND Hora < CDate(rsConfig("HR_TER_FIM")) Then
			strMensagem = "Medição só pode ser atualizada apartir de terça " & rsConfig("HR_SEG_FIM")
		    VerificaHB = false
		End If
	End If
	...
	If rsConfig("FLAG_QUA") = true Then
		If Dia = 4 AND Hora > CDate(rsConfig("HR_QUA_INICIO")) AND Hora < CDate(rsConfig("HR_QUA_FIM")) Then
			strMensagem = "Medição só pode ser atualizada apartir de quarta " & rsConfig("HR_SEG_FIM")
		    VerificaHB = false
		End If
	End If
...
if VerificaHB then
	...
	If trim(session("Login")) = "" Then
		Response.Redirect ("./Erro.asp?Erro=Sua sessão expirou. Por-favor, logue-se novamente.&Voltar=true&IrPara=./CUT_LOGIN.ASP?hidOrigem=./CUT_Medicao_Detalhe.asp")
	End if
...
'	End if
	...
	if strOperacao <> "" Then
		...
		response.Redirect("./CUT_Confirmacao_Medicao.asp?strUsuario=" & strUsuario & "&strPagina=CUT_Medicao_Detalhe.asp")
...
'*********************************************************************************
	...
	<FORM name="frmMedicao_Detalhe_CUT" id="frmMedicao_Detalhe_CUT" action="CUT_Medicao_Detalhe.asp" method="post">
...
{
	var intCont	= (document.frmMedicao_Detalhe_CUT.hidCont.value - 1)

	if (intCont == 0)
	{

		var conteudo = document.frmMedicao_Detalhe_CUT.txtComplete.value
		var DtFim = document.frmMedicao_Detalhe_CUT.hiddtFinish.value
		var DtInicio = document.frmMedicao_Detalhe_CUT.hiddtStart.value
		var TpLogin = document.frmMedicao_Detalhe_CUT.hidTpLogin.value

		if (jTrim(conteudo) == "")
		{
			alert("O conteúdo do campo %Comp invalido. deve ser numérico.");
			document.frmMedicao_Detalhe_CUT.txtComplete.value = document.frmMedicao_Detalhe_CUT.hidComplete(num).value;
			document.frmMedicao_Detalhe_CUT.txtComplete.focus();
			return false;
		}

		if (parseInt(conteudo) < parseInt(document.frmMedicao_Detalhe_CUT.hidComplete.value))
		{
			alert("O conteúdo do campo %Comp invalido. deve ser maior que o atual.");
			document.frmMedicao_Detalhe_CUT.txtComplete.value = document.frmMedicao_Detalhe_CUT.hidComplete.value;
			document.frmMedicao_Detalhe_CUT.txtComplete.focus();
			return false;
		}

	/*	if (parseInt(conteudo) != parseInt(document.frmMedicao_Detalhe_CUT.hidComplete(num).value))
		{
			if ((TpLogin == 'P') && (conteudo > 50))
			{
				alert("O conteúdo do campo %Comp invalido para o usuário. deve ser menor ou igual a 50.");
				document.frmMedicao_Detalhe_CUT.txtComplete(num).value = document.frmMedicao_Detalhe_CUT.hidComplete(num).value;
				document.frmMedicao_Detalhe_CUT.txtComplete(num).focus();
				return false;
			}

			if ((TpLogin == 'S') && (parseInt(document.frmMedicao_Detalhe_CUT.hidComplete(num).value) < 50))
			{
	...
	*/	
		if (parseInt(document.frmMedicao_Detalhe_CUT.hidProj.value)!= 8)
		{
			if (DtInicio == DtFim)
			{ 
	...
	{
		var conteudo = document.frmMedicao_Detalhe_CUT.txtComplete(num).value
		var DtFim = document.frmMedicao_Detalhe_CUT.hiddtFinish(num).value
		var DtInicio = document.frmMedicao_Detalhe_CUT.hiddtStart(num).value
		var TpLogin = document.frmMedicao_Detalhe_CUT.hidTpLogin(num).value
	...
	*/	
		if (parseInt(document.frmMedicao_Detalhe_CUT.hidProj(num).value)!= 8)
		{
			if (DtInicio == DtFim)
			{ 
				if ((conteudo != 0) && (conteudo != 100))
...
function Confirmar()
{
	...
	document.frmMedicao_Detalhe_CUT.style.cursor = "wait";
			
	document.frmMedicao_Detalhe_CUT.hidDados.value = "";
	document.frmMedicao_Detalhe_CUT.hidOperacao.value = "";
	
	if (intCont == 0)
	{
		for(var i = 0; i <= intCont;i++)
		{
			if (document.frmMedicao_Detalhe_CUT.txtComplete.value != document.frmMedicao_Detalhe_CUT.hidComplete.value)
			{
				if (strAux == "")
				{
					strAux = document.frmMedicao_Detalhe_CUT.hidProj.value + "|" + document.frmMedicao_Detalhe_CUT.hidId.value + "|" + document.frmMedicao_Detalhe_CUT.hidUid.value + "|" + document.frmMedicao_Detalhe_CUT.txtComplete.value;
				}	
				else
				{
					strAux = strAux + ";" + document.frmMedicao_Detalhe_CUT.hidProj.value + "|" + document.frmMedicao_Detalhe_CUT.hidId.value + "|" + document.frmMedicao_Detalhe_CUT.hidUid.value + "|" + document.frmMedicao_Detalhe_CUT.txtComplete.value;
				}
			}
		}
	}
	else
	{
		for(var i = 0; i <= intCont;i++)
		{
			if (document.frmMedicao_Detalhe_CUT.txtComplete(i).value != document.frmMedicao_Detalhe_CUT.hidComplete(i).value)
			{
				if (strAux == "")
				{
					strAux = document.frmMedicao_Detalhe_CUT.hidProj(i).value + "|" + document.frmMedicao_Detalhe_CUT.hidId(i).value+ "|" + document.frmMedicao_Detalhe_CUT.hidUid(i).value + "|" + document.frmMedicao_Detalhe_CUT.txtComplete(i).value;
				}	
				else
				{
					strAux = strAux + ";" + document.frmMedicao_Detalhe_CUT.hidProj(i).value + "|" + document.frmMedicao_Detalhe_CUT.hidId(i).value + "|" + document.frmMedicao_Detalhe_CUT.hidUid(i).value + "|" + document.frmMedicao_Detalhe_CUT.txtComplete(i).value;
				}
			}
		}
	}
	...
	if (strAux != "")
	{
		document.frmMedicao_Detalhe_CUT.hidDados.value = strAux;
		document.frmMedicao_Detalhe_CUT.hidOperacao.value = 'A';
		document.frmMedicao_Detalhe_CUT.style.cursor = "";
		document.frmMedicao_Detalhe_CUT.submit();
	}
	else
	{
