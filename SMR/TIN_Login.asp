	...
	if (strUsuario <> "" and strSenha <> "") then
		...
		if rs.EOF Then
			...
			if rs("Senha") <> strSenha Then
				...
				session("Usuario") = trim(rs("Nome"))
				session("Perfil") = rs("Perfil")
				Response.Redirect ("./TIN_Medicao_Filtro.asp?pCaminho=Medição")
			end if
...
<tr bgcolor=#6699cc>
	<th>
	<font color=White size="1" face="Georgia, Times New Roman, Times, serif">&nbsp;Informe seus dados para entrar no Teste Integrado...</font>
	</th>
</tr>
<tr>
	<td>
...
function Confirmar()
{
	document.frmLogin.action = "TIN_Login.asp";
