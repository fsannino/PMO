					...
					<font face="Arial" size="1">
						<%If intCont = 0 Then%>
							<%If cint(rs("Andamento")) = 0 Then%>
								<%=rs("Andamento")%>&nbsp;
							<%Else%>
								<a href="JavaScript:abreJanelaRelIssues('PMO_Relatorio_Issues_Detalhado.asp?strEquipe=<%=rs("Cod_Equipe")%>&strData=<%=rs("Data_Score")%>&strTipo=A')">
								<%=rs("Andamento")%>&nbsp;
								</a>
							<%End If%>
						<%Else%>
					...
					<font face="Arial" size="1">
						<%If intCont = 0 Then%>
							<%If cint(rs("Atrasadas10")) = 0 Then%>
			   					<%=rs("Atrasadas10")%>&nbsp;
							<%Else%>
								<a href="JavaScript:abreJanelaRelIssues('PMO_Relatorio_Issues_Detalhado.asp?strEquipe=<%=rs("Cod_Equipe")%>&strData=<%=rs("Data_Score")%>&strTipo=AT10')">
			   					<%=rs("Atrasadas10")%>&nbsp;
								</a>
							<%End If%>
						<%Else%>
					...
					<font face="Arial" size="1">
						<%If intCont = 0 Then%>
							<%If cint(rs("AtrasadasMais10")) = 0 Then%>
			   					<%=rs("AtrasadasMais10")%>&nbsp;
							<%Else%>	
								<a href="JavaScript:abreJanelaRelIssues('PMO_Relatorio_Issues_Detalhado.asp?strEquipe=<%=rs("Cod_Equipe")%>&strData=<%=rs("Data_Score")%>&strTipo=ATM10')">
			   					<%=rs("AtrasadasMais10")%>&nbsp;
								</a>
							<%End If%>
						<%Else%>
...
function Confirmar()
{
	...
	document.frmRelatorio_Scorecard.action = "PMO_Relatorio_Scorecard.asp";
