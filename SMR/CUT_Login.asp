	...
	if (strUsuario <> "" and strSenha <> "") then
		...
		if rs.EOF Then
			...
			if rs("Senha") <> strSenha Then
				...
				session("Usuario") = trim(rs("Nome_Usuario"))

				If Trim(strOrigem) = "" Then 
					strOrigem = "./CUT_Medicao_Detalhe.asp"
				Else
...
<tr bgcolor=#6699cc>
	<th>
	<font color=White size="1" face="Georgia, Times New Roman, Times, serif">&nbsp;Informe seus dados para entrar no CutOver...</font>
	</th>
</tr>
<tr>
	<td>
...
<INPUT type="hidden" id=hidOrigem name=hidOrigem value=<%=strOrigem%>>

</form>
<SCRIPT language=JavaScript>
function Confirmar()
{
	document.frmLogin.action = "CUT_Login.asp";
