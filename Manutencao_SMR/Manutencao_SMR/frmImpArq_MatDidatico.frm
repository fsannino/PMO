...
Begin VB.Form frmImpArq_MatDidatico 
   Caption         =   "Importação do Arquivo Material Didático"
   ClientHeight    =   2115
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6060
   Icon            =   "frmImpArq_MatDidatico.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   2115
   ...
   End
End
Attribute VB_Name = "frmImpArq_MatDidatico"
...
Dim strDir As String
    ...
    If Dir(strArq) = "" Then
        MsgBox "Diretório ou arquivo não existe.", vbCritical + vbOKOnly, "Importação do Arquivo do Material Didático"
        Exit Sub
    End If
    
    If UCase(Right(strArq, 4)) <> ".XLS" Then
        MsgBox "Arquivo inválido.", vbCritical + vbOKOnly, "Importação do Arquivo do Material Didático"
        Exit Sub
    End If
    ...
    If ImportarArq(strArq) Then
        MsgBox "Exportação gerada com sucesso.", vbInformation + vbOKOnly, "Geração do Arquivo para Material Didático"
    Else
        MsgBox "Erro na geração do arquivo.", vbCritical + vbOKOnly, "Geração do Arquivo para Material Didático"
    End If
...
On Error GoTo ErrcmdProcura
    ...
    CommonDialog1.DialogTitle = "Importação do Arquivo Material Didático"
...
On Error GoTo ErrImportarArq
        ...
        strArqSaida = Mid(strArquivo, 1, InStrRev(strArquivo, "\")) & "INC_MAT_" & Format(Year(Now()), "0000") & Format(Month(Now()), "00") & Format(Day(Now()), "00")
        ...
        wrkSheetDados1.Cells(1, 3).Value = "Completeza Cronograma"
        wrkSheetDados1.Cells(1, 4).Value = "Completeza Planilha"
        wrkSheetDados1.Cells(1, 5).Value = "Status"
        ...
        For intLinha = 2 To intTotal
            ...
            intCompleteza = wrkSheetDados.Cells(intLinha, 3).Value
            strStatus = wrkSheetDados.Cells(intLinha, 4).Value
            ...
            If Trim(intCod_Proj) <> "" And Trim(intUID) <> "" Then
                ...
                If Not rs.EOF Then
                    ...
                    If intCompleteza < rs("TASK_PCT_COMP") Then
                        ...
                        wrkSheetDados1.Cells(intLinhaAux, 3).Value = rs("TASK_PCT_COMP")
                        wrkSheetDados1.Cells(intLinhaAux, 4).Value = intCompleteza
                        wrkSheetDados1.Cells(intLinhaAux, 5).Value = strStatus
    
                        intLinhaAux = intLinhaAux + 1
                    
                    Else
                    
                        strSQL = "EXEC SP_ATUALIZAR_MATERIAL_DIDATICO " & intCod_Proj & ", " & intUID & ", " & intCompleteza & " "
