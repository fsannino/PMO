...
Begin VB.Form frmAtualizarPerc_EP_SMR 
   Caption         =   "Atualização do Percentual EP -> SMR"
   ClientHeight    =   1425
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6060
   Icon            =   "frmAtualizarPerc_EP_SMR.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   1425
   ...
   End
End
Attribute VB_Name = "frmAtualizarPerc_EP_SMR"
...
Private Sub cmdOK_Click()
    ...
    If AtualizarPerc_EP_SMR() Then
        MsgBox "Atualização do Percentual EP -> SMR gerada com sucesso.", vbInformation + vbOKOnly, "Atualização do Percentual EP -> SMR"
    Else
        MsgBox "Erro na Atualização do Percentual EP -> SMR.", vbCritical + vbOKOnly, "Atualização do Percentual EP -> SMR"
    End If
...
Private Function AtualizarPerc_EP_SMR() As Boolean
...
On Error GoTo ErrAtualizarPerc_EP_SMR

        fraAtualizacaoTar.Caption = "Atualização de Tarefas EP -> PMO"
        fraAtualizacaoTar.Refresh
        
        AtualizarPerc_EP_SMR = False
        
        Screen.MousePointer = vbHourglass
        
        strArqSaida = "C:\INC_AT_EP_PMO" & Format(Year(Now()), "0000") & Format(Month(Now()), "00") & Format(Day(Now()), "00")
        
        strArqSaida1 = "C:\INC_TS_EP_PMO" & Format(Year(Now()), "0000") & Format(Month(Now()), "00") & Format(Day(Now()), "00")
        ...
        'Cria titulos das colunas da tabela INC_AT_EP_PMO
        wrkSheetDados.Cells(1, 1).Value = "Projeto Orig. EP."
        wrkSheetDados.Cells(1, 2).Value = "UID Orig. EP."
        wrkSheetDados.Cells(1, 3).Value = "Tarefa Orig. EP."
        wrkSheetDados.Cells(1, 4).Value = "%Complete Orig. EP."
        ...
        'Cria titulos das colunas da tabela INC_TS_PMO_EP
        wrkSheetDados1.Cells(1, 1).Value = "Projeto Orig. EP."
        wrkSheetDados1.Cells(1, 2).Value = "UID Orig. EP."
        wrkSheetDados1.Cells(1, 3).Value = "Tarefa Orig. EP."
        ...
        'Seleciona dados da tabela De/Para PMO->EP
        strSql = "SP_LISTAR_ATUALIZA_PERC_EP_SMR "
        ...
        Do While Not rs1.EOF
            ...
            rs2.Open strSql, dbConexaoEP, adOpenStatic, adLockReadOnly, adCmdText
            ...
            If Not rs2.EOF Then
                ...
                If Not rs3.EOF Then
                    ...
                    'Se tarefa origem for atividade e tarefa destino for sumaria não atualizar e salvar na planilha INC_TS_PMO_EP
...
'                            strSql = "SP_ATUALIZAR_TAREFAS " & Trim(Str(rs1("Cod_Proj_Dest"))) & ", " & Trim(Str(rs1("Cod_UID_Dest"))) & ", " & Trim(Str(rs2("TASK_PCT_COMP")))
                        ...
                        Else
                            'Se o % de origem for menor a % de destino salvar na planilha INC_AT_PMO_EP
...
'                            strSql = "SP_ATUALIZAR_TAREFAS " & Trim(Str(rs1("Cod_Proj_Dest"))) & ", " & Trim(Str(rs1("Cod_UID_Dest"))) & ", " & Trim(Str(rs2("TASK_PCT_COMP")))
        ...
        AtualizarPerc_EP_SMR = True
        
        Exit Function
        
ErrAtualizarPerc_EP_SMR:
