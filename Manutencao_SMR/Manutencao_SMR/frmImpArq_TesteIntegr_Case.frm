...
Begin VB.Form frmImpArq_TesteIntegr_Case 
   Caption         =   "Importação do Arquivo Teste Integrado Cases"
   ClientHeight    =   2115
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6060
   Icon            =   "frmImpArq_TesteIntegr_Case.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   2115
   ...
   End
End
Attribute VB_Name = "frmImpArq_TesteIntegr_Case"
...
Dim strDir As String
    ...
    If Dir(strArq) = "" Then
        MsgBox "Diretório ou arquivo não existe.", vbCritical + vbOKOnly, "Importação do Arquivo Teste Integrado Cases"
        Exit Sub
    End If

    If UCase(Right(strArq, 4)) <> ".XLS" Then
        MsgBox "Arquivo inválido.", vbCritical + vbOKOnly, "Importação do Arquivo Teste Integrado Cases"
        Exit Sub
    End If
    ...
    If ImportarArq(strArq) Then
        MsgBox "Importação gerada com sucesso.", vbInformation + vbOKOnly, "Geração do Arquivo Teste Integrado Cases"
    Else
        MsgBox "Erro na Importação do arquivo.", vbCritical + vbOKOnly, "Geração do Arquivo Teste Integrado Cases"
    End If
...
On Error GoTo ErrcmdProcura
    ...
    CommonDialog1.DialogTitle = "Importação do Arquivo Teste Integrado Cases"
...
On Error GoTo ErrImportarArq
        ...
        strArqSaida = Mid(strArquivo, 1, InStrRev(strArquivo, "\")) & "Inc_TI_CS_" & Format(Year(Now()), "0000") & Format(Month(Now()), "00") & Format(Day(Now()), "00")
        ...
        wrkSheetDados1.Cells(1, 3).Value = "Status"
        ...
        For intLinha = 2 To intTotal
            ...
            strStatus = wrkSheetDados.Cells(intLinha, 3).Value
            ...
            If Trim(intCod_Proj) <> "" And Trim(intUID) <> "" Then
                ...
                If Not rs.EOF Then
                    ...
                    strSQL = "EXEC SP_ATUALIZAR_TESTE_INTEGRADO_CASE " & intCod_Proj & ", " & intUID & ", '" & strStatus & "' "
                ...
                Else
                    ...
                    wrkSheetDados1.Cells(intLinhaAux, 3).Value = strStatus
