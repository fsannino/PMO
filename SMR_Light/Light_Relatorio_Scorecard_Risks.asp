...
intTotalGeral = 0
    ...
    With cmdResultado
        ...
        .CommandType = 4
		.CommandTimeout = 480
        .CommandText = "SP_LISTAR_SCORECARD_RISKS"
	...
	<FORM name="frmRelatorio_Scorecard_Risks" id="frmRelatorio_Scorecard_Risks" method="post">
	...
	<table width="100%" border="0">
		<tr>
			...
			<td width="40%" align="center" height="10px">
				<p><b><font size="3" face="Georgia, Times New Roman, Times, serif" color="#666666">Scorecard de Riscos</font></b></p>
			</td>
			<td width="30%" height="10px">&nbsp;</td>
		</tr>
	</table>
					...
					<font face="Arial" size="1">
						<%If intCont = 0 Then%>
							<%If cint(rs("Andamento")) = 0 Then%>
								<%=rs("Andamento")%>&nbsp;
							<%Else%>
								<a href="JavaScript:abreJanelaRelRisks('Light_Relatorio_Risks_Detalhado.asp?strEquipe=<%=rs("Cod_Equipe")%>&strData=<%=rs("Data_Score")%>&strTipo=A')">
								<%=rs("Andamento")%>&nbsp;
								</a>
							<%End If%>
						<%Else%>
					...
					<font face="Arial" size="1">
						<%If intCont = 0 Then%>
							<%If cint(rs("Atrasadas10")) = 0 Then%>
			   					<%=rs("Atrasadas10")%>&nbsp;
							<%Else%>
								<a href="JavaScript:abreJanelaRelRisks('Light_Relatorio_Risks_Detalhado.asp?strEquipe=<%=rs("Cod_Equipe")%>&strData=<%=rs("Data_Score")%>&strTipo=AT10')">
			   					<%=rs("Atrasadas10")%>&nbsp;
								</a>
							<%End If%>
						<%Else%>
					...
					<font face="Arial" size="1">
						<%If intCont = 0 Then%>
							<%If cint(rs("AtrasadasMais10")) = 0 Then%>
			   					<%=rs("AtrasadasMais10")%>&nbsp;
							<%Else%>	
								<a href="JavaScript:abreJanelaRelRisks('Light_Relatorio_Risks_Detalhado.asp?strEquipe=<%=rs("Cod_Equipe")%>&strData=<%=rs("Data_Score")%>&strTipo=ATM10')">
			   					<%=rs("AtrasadasMais10")%>&nbsp;
								</a>
							<%End If%>
						<%Else%>
...
function Confirmar()
{
	document.frmRelatorio_Scorecard_Risks.strHist.value = "S";
	document.frmRelatorio_Scorecard_Risks.action = "Light_Relatorio_Scorecard_Risks.asp";
	document.frmRelatorio_Scorecard_Risks.submit();
}
