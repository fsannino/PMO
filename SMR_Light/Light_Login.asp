	...
	if (strUsuario <> "" and strSenha <> "") then
		...
		if rs.EOF Then
			...
			if rs("Senha") <> strSenha Then
				...
				session("Todos") =  rs("Todos")
					
				If Trim(strOrigem) = "" Then 
					strOrigem = "./Light_Medicao_Detalhe.asp"
				Else
...
function realizarLogin() {
	if (isCampoPreenchido(frmLogin.txtUsuario, "Usuário") &&
		...
		frmLogin.action = "./Light_Login.asp";
		return true;
	} else {
		return false;
	}
...
<tr bgcolor=#6699cc>
	<th>
	<font color=White size="1" face="Georgia, Times New Roman, Times, serif">&nbsp;Informe seus dados para entrar no SMR BRACUSS</font>
	</th>
</tr>
<tr>
	<td>
...
function Confirmar()
{
	document.frmLogin.action = "Light_Login.asp?hidOrigem=<%=hidOrigem%>";
